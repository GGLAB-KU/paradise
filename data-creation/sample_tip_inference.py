...
def main():
    ...
    tips = (list(tips_file.values()))
    tips = [tip for tip_list in tips for tip in tip_list]

    embeddings = []
    for tip in tqdm(tips):
        token_indexes = get_verb_and_noun_token_indexes(tip, tokenizer)
        emb = get_mean_of_the_verb_and_noun_tokens(tip, token_indexes, model, tokenizer)
    ...
    for tip in tqdm(tips):
        try:
            tip_verb_token_indexes = get_verb_and_noun_token_indexes(tip, tokenizer)
            tip_embedding = get_mean_of_the_verb_and_noun_tokens(tip, tip_verb_token_indexes, model, tokenizer)

            k = 30
            D, I = index.search(tip_embedding, k)
            negative_candidates = []
            negative_candidates_similarities = []

            for i in range(k):
                token_indexes = get_verb_and_noun_token_indexes(tips[I[0][i]], tokenizer)
                if token_indexes != []:
                    retrieved_goal_embedding = get_mean_of_the_verb_and_noun_tokens(tips[I[0][i]], token_indexes, model, tokenizer)
                    cosine_scores = util.cos_sim(tip_embedding, retrieved_goal_embedding)
                    if (tip != tips[I[0][i]]):
                        negative_candidates.append(tips[I[0][i]])
            ...
            goal = [k for k, v in tips_file.items() if tip in v][0]
            tip_inference[len(tip_inference)] = {"goal": goal, "positive_candidate":tip, "negative_candidates":negative_candidates, "negative_candidates_similarities":negative_candidates_similarities}
            torch.cuda.empty_cache()
        except:
            pass

    with open(args.save_path, "w", encoding="utf-8") as f:
        json.dump(tip_inference, f, ensure_ascii=False, indent=3)
